...
const FormTemplate = ({ algorithm, setOutput, calculateAlgorithm }) => {
  ...
  const fieldsByAlgorithm = {
    ...
    MLFQ: ["Process ID", "Arrival Time", "Burst Time"],
    HybridScheduler: ["Process ID", "Arrival Time", "Burst Time", "Priority"],
  };
  ...
  const handleSubmit = (e) => {
    ...
    const dataToSubmit =
      algorithm === "RoundRobin" || algorithm === "MLFQ" || algorithm === "HybridScheduler"
        ? { processes, quantum }
        : { processes };
          ...
          <div key={index} className="template-process-row">
            ...
            {fields.includes("Priority") && (
              <input
                ...
                min={algorithm === "HybridScheduler" ? "0" : "1"}
                required
              />
            )}
            <button
